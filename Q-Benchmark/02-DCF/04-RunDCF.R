...
output_dir <- "./Q-Benchmark/02-DCF/00-Data/03-Results/DCF"
...
for (name in names(pm_datasets)) {
  ...
  out_path <- file.path(
    output_dir,
    paste0("DCF_ProteoMaker", name, "_results.csv")
  )
  ...
  fwrite(
    all_res,
    file.path(output_dir, paste0("DCF_ProteoMaker", name, "_results.csv"))
  )
...
fwrite(PM_summary, file.path(output_dir, "Summary_DCF_ProteoMaker.csv"))
...
for (name in names(cx_datasets)) {
  ...
  out_path <- file.path(
    output_dir,
    paste0("DCF_Complex_", name, "_results.csv")
  )
  if (file.exists(out_path)) {
    cat("Skipping", name, "- result already exists\n")
    next
  ...
  fwrite(
    res,
    file.path(output_dir, paste0("DCF_Complex_", name, "_results.csv"))
  )
...
fwrite(cx_summary, file.path(output_dir, "Summary_DCF_Complex.csv"))
