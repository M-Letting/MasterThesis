...
output_dir <- "03-Clustering/00-Data/03-DCFConstraints"
...
################################################################################
## Run VSClust on datasets from 02-DCF with DCF constraints ####################
################################################################################
...
# Get accessions for top 10 proteins with most peptides
high_no_na_data <- fread(
  "./02-DCF/00-Data/03-Results/RPC/RPC_ProteoMakerHighNoNA_results.csv"
)
...
# cat(paste("#", 80), "\n")
# cat(
#   "Running VSClust with DCF constraints on ProteoMaker datasets (RPC results, variant: ",
#   cannot_link_variant,
#   ")\n"
# )
...
#       constraint_matrix <- dCF_to_vsclust_constraints(
#         data = protein_data,
...
#   file.path(
#     output_dir,
#     paste0("/01-PM_RPC_DCF_ClusteringResults_", cannot_link_variant, ".rds")
#   )
# )
...
# Get top 25 proteins by peptide count from HighNoNA (must match CLM matrices)
high_no_na_ref <- if ("HighNoNA" %in% names(datasets)) {
  datasets[["HighNoNA"]]
} else {
  datasets[["HighNA"]]
}
high_no_na_data <- fread(high_no_na_ref)
top_proteins_high_no_na <- high_no_na_data[,
  .N,
  by = Accession
][order(-N)][1:25, Accession]
...
for (th in constraints_thresholds) {
  ...
  for (name in names(datasets)) {
    ...
    for (protein in top_proteins_high_no_na) {
      ...
      # Convert to clustering constraints
      constraint_matrix <- dCF_to_constraints(
        data = protein_data,
        cannotlink_matrix = CLM,
        dCF_col = "dCF",
        id_cols = c("Accession", "Peptide_ID"),
      ...
      # Find number of clusters
      nClust <- ncol(constraint_matrix)
      if (nClust == 0 || nrow(constraint_matrix) == 0) {
        cat("  No clusters available after applying constraints. Skipping.\n")
        next
      }
...
# Save ProteoMaker DCF results
saveRDS(
  PM_DCF,
  file.path(
    output_dir,
    paste0(
      "01-PM_DCF_ClusteringResults_",
      cannot_link_variant,
      ".rds"
    )
  )
...
summary_output_file <- file.path(
  output_dir,
  paste0("/01-PM_DCF_Summary_", cannot_link_variant, ".csv")
)
fwrite(PM_summary, summary_output_file)

# Log total time taken for ProteoMaker DCF datasets
total_time <- round(
  as.numeric(difftime(Sys.time(), PM_DCF_start_time, units = "mins")),
  2
)
cat(paste(
  "Total time taken for ProteoMaker DCF datasets:",
  total_time,
  "minutes\n"
))
cat(paste(rep("#", 80), collapse = ""), "\n")
...
# cat(
#   "Running VSClust with DCF constraints on Protein complex datasets (RPC results, variant: ",
#   cannot_link_variant,
#   ")\n"
# )
...
#   file.path(
#     output_dir,
#     paste0("01-PC_RPC_DCF_ClusteringResults_", cannot_link_variant, ".rds")
#   )
# )
...
cat(
  "Running VSClust with DCF constraints on Protein complex datasets (DCF results, variant: ",
  cannot_link_variant,
  ")\n"
)
...
fwrite(
  ...
  file.path(
    output_dir,
    paste0("01-PC_DCF_Summary_", cannot_link_variant, ".csv")
  )
)
