...
for (i in seq_along(data_ids)) {
  ...
  # Step 2b: Log2 transform and median normalize intensity columns
  cat("  Applying log2 transformation and median normalization...\n")

  # Log transform intensity columns (log2)
  for (col in intensity_cols) {
    wide_data[[col]] <- log2(wide_data[[col]])
  }
  ...
  for (protein in unique_proteins) {
    ...
    tryCatch(
      {
        result <- discover_complexoforms_new(
